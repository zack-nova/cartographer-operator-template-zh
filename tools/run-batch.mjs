...
var SourceResolveError = class extends Error {
...
async function resolveRepoTask(batchContext, repoTask) {
...
async function createSchemaRegistry(schemaDir) {
  ...
  for (const schemaFile of SCHEMA_FILES) {
    const schemaPath = path2.join(schemaDir, schemaFile);
...
async function loadYamlFile(filePath) {
  return parse(await readFile2(filePath, "utf8"));
}
...
// src/lib/input-validation.mjs
async function loadAndValidateReposInput(inputFilePath, schemaRegistry) {
...
function assertUniqueTargetSlugs(repos) {
...
function resolvePathFromInput(inputFilePath, configuredPath) {
  return path3.resolve(path3.dirname(inputFilePath), configuredPath);
}
...
var LOCAL_LOCATOR_PATTERN = /((?:\/|\.\.?\/)[^`\s]+)/u;
var REF_CODE_SPAN_PATTERN = /(?:\bon\b|\bref\b)\s+`([^`]+)`/iu;
var REF_BARE_PATTERN = /(?:\bon\b|\bref\b)\s+([A-Za-z0-9._/-]+)/iu;
var TARGET_CODE_SPAN_PATTERN = /(?:\bas\b|\btarget_slug\b|\btarget\b)\s+`([^`]+)`/iu;
var TARGET_BARE_PATTERN = /(?:\bas\b|\btarget_slug\b|\btarget\b)\s+([A-Za-z0-9._-]+)/iu;
...
function deriveTargetSlug(repoLocator) {
  ...
  const repoPath = normalizedLocator.includes(":") && !normalizedLocator.includes("://") ? normalizedLocator.split(":").at(-1) : normalizedLocator;
  const pathSegment = repoPath?.split("/").at(-1);
  if (pathSegment === void 0 || pathSegment.trim() === "") {
...
function extractExplicitTargetSlug(body) {
  return TARGET_CODE_SPAN_PATTERN.exec(body)?.[1] ?? TARGET_BARE_PATTERN.exec(body)?.[1];
}
...
function createBatchId() {
  return (/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-");
}
...
function renderBatchSummary(batchId, reports) {
  ...
  for (const report of reports) {
    const suffix = report.error === void 0 ? "" : ` (${report.error.code}: ${report.error.message})`;
  ...
  try {
    for (const repoTask of reposInput.repos) {
      ...
      try {
        ...
        if (repoTask.harness_id !== void 0) {
          bootstrapArgs.push("--harness-id", repoTask.harness_id);
        }
        if (repoTask.orbit_id !== void 0) {
